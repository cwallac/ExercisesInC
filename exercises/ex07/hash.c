...
{
    int hashedKey = key->hash(key->key);
    map->lists[hashedKey] = prepend(key,value,map->lists[hashedKey]);
...
Value *map_lookup(Map *map, Hashable *key)
{
    // FILL THIS IN!
    int hashedKey = key->hash(key->key);
    return list_lookup(map->lists[hashedKey], key);
}
