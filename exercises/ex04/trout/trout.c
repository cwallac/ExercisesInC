...
int pipefd[2];              /* the pipe for the alarm handler */
        ...
        /* length of the data in a datagram */
...
int process_ip (struct ip *ip, int len)
{
  ...
  if (icmp->icmp_type == ICMP_TIMXCEED) {
    //So the error previously here doesn't exist and this is either
    //ICMP_TIMXCEED_INTRANS or ICMP_TIMXCEED_REASS, it is unclear which
